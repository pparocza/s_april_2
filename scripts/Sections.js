...
function playMalletKeys(startTime, stopTime, now){
	...
	// STOP
		...
		sK3.stopAtTime(stopTime+now);
		sK4.stopAtTime(stopTime+now);
...
function bassLineSection(fund, startTime, now){
	...
	// A
		bassSequence(startTime, now, eB, sL, fund, c1, onsetBase, onsetExpArray, durationBase, durationExpArray, 0.03, 0.04);
		// fxSequence(startTime, now, eBFX1A, fxSL);


	// B
		bassSequence(startTime+8, now, eB, sL, fund, c2, onsetBase, onsetExpArray, durationBase, durationExpArray, 0.03, 0.04);
		// fxSequence(startTime+8, now, eBFX1A, fxSL);


	// A
		bassSequence(startTime+16, now, eB, sL, fund, c1, onsetBase, onsetExpArray, durationBase, durationExpArray, 0.04, 0.07);
		fxSequence(startTime+16, now, eBFX1A, fxSL);


	// B
		bassSequence(startTime+24, now, eB, sL, fund, c2, onsetBase, onsetExpArray, durationBase, durationExpArray, 0.04, 0.07);
		fxSequence(startTime+24, now, eBFX1A, fxSL);

	// A
		bassSequence(startTime+32, now, eB, sL, fund, c1, onsetBase, onsetExpArray, durationBase, durationExpArray, 0.06, 0.0825);
		fxSequence(startTime+32, now, eBFX1A, fxSL);

	// B
		bassSequence(startTime+40, now, eB, sL, fund, c2, onsetBase, onsetExpArray, durationBase, durationExpArray, 0.06, 0.0825);
		fxSequence(startTime+40, now, eBFX1A, fxSL);

	// A
		bassSequence(startTime+48, now, eB, sL, fund, c1, onsetBase, onsetExpArray, durationBase, durationExpArray, 0.05, 0.07);
		fxSequence(startTime+48, now, eBFX1A, fxSL);


	// B
		bassSequence(startTime+56, now, eB, sL, fund, c2, onsetBase, onsetExpArray, durationBase, durationExpArray, 0.05, 0.07);
		fxSequence(startTime+56, now, eBFX1A, fxSL);
...
function playFlutterXylophone(startTime, now){
	...
	for(var k=0; k<3; k++){
		//
		if(k==0){fKChordPlayer(time, k, c1, fund, mm);}
		else if(k==1){fKChordPlayer(time, k, c2, fund, mm);}
		else if(k==2){fKChordPlayer(time, k, c1, fund, mm);}
		// else if(k==3){fKChordPlayer(time, k, c2, fund, mm);}
	}

	mm.stopAtTime(startTime+now+24);
...
function playNoiseTone(startTime, now, fund){
	...
	// OFF
	nT.stopAtTime(startTime+now+16);
